...
% We only consider $C_0$ and finite measures!
% Don't care of $C_c$!
% If $\Omega$ is no metrizable, then just replace finite measures to regular finite measures!
...
\item $\mu^*|_\cT=\rho$.
\item $\cB\subset\cM$.
\item $I(f)=\int f\,d\mu$ for $f\in C(\Omega)$.
\item The map $\mu\mapsto(f\mapsto\int f\,d\mu)$ is injective.
\end{parts}
\end{prb}
\begin{pf}
(a)
