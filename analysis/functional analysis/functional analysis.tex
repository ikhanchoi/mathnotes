...
\begin{parts}
\item
$f$ is weakly continuous if and only if it is strongly continuous, and in this case we have $f=\sum_{i=1}^n\omega_{x_i,y_i}$ for some $(x_i)_{i=1}^n,(y_i)_{i=1}^n\subset H$.
\end{parts}
\end{prb}
\begin{pf}
...
Let $T_\alpha$ be a net of bounded self-adjoint operators on $H$ which is increasing and bounded above.
Then, $T_\alpha$ converges strongly.
\end{prb}
\begin{pf}
Define $T$ such that
...
Then, $T_\alpha\to T$ weakly by definition, and strongly by because the net is increasing.
\end{pf}
...
p_{\bar x,\bar y}^{\sigma w}(T)=\left|\sum_i\<Tx_i,y_i\>\right|\]
...
\begin{parts}
\item
Jordan decomposition: A self-adjoint linear functional is the difference of two positive linear functional.
\end{parts}
\end{prb}
\begin{pf}

\[\{\omega\in A^*:\omega(a^*)=\bar{\omega(a)},\ \|\omega\|\le1\}=\conv(S(A)\cup-S(A)).\]
Krein-Milman
...
Then, the image of $ap$ is contained in $\bar{A\xi}$, we have $pap=ap$ and $pa^*p=a^*p$ for all $a\in A$ by the self-adjointness of $A$.
...
\end{prb}
\begin{pf}
Existence of range projections.
Let $x\in M$.
...
every von neumann algebra is unital: also can be proved by Krein-Milman

polar decomposition

normal linear functionals form a predual of $M$.
