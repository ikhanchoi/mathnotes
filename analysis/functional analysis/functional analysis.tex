...
An operator $K\in B(H)$ is
\[K=\sum_i\lambda_i\theta_{\xi_i,\eta_i},\qquad\Tr(\cdot K)=\sum_i\lambda_i\omega_{\xi_i,\eta_i},\]
for $(\lambda_i)\in\ell^1$ and orthonormal sequences $(\xi_i)_i$ and $(\eta_i)_i$
\end{prb}
...
\item $f=\sum_{i=1}^n\omega_{\xi_i,\eta_i}$ for some $\xi_i,\eta_i\in H$.
\end{parts}
\end{prb}
\begin{pf}
...
Let $(T_\alpha)_\alpha\subset B(H)$ and suppose $T_\alpha\to T$ strongly.
\end{prb}

\begin{prb}[Strict topology]
Let $H$ be a Hilbert space.
Let $(T_\alpha)_\alpha\subset B(H)$ and $K\in K(H)$.
\begin{parts}
...
Let $b:=a^*a$.
If we consider the case $\cA=B(H)$, then one of the properties characterizing $b$ is that $\<b\xi,\xi\>\ge0$ for all $\xi\in H$.
Let $b=b_+-b_-$.
Then,
\[0\le\<bb_-\xi,b_-\xi\>=-\<b_-^3\xi,\xi\>\le0\]
implies $b_-=0$.
Now we assume $\cA$ is defined axiomatically.
Then,
\[(ab_-)^*(ab_-)=b_-bb_-=-b_-^3\le0\]
implies $(ab_-)(ab_-)^*\le0$ and
\[0\le(ab_-)^*(ab_-)+(ab_-)(ab_-)^*\le0.\]
Thus we have $ab_-=0$ and $b_-^3=0$.
...
(e)
Since $\bar\cA^w$ is closed convex, $\bar\cA^s=\bar\cA^w$.
Also, $\cA''$ is weakly closed, $\bar\cA^s\subset\cA''$.
\end{pf}
