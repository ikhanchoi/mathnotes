...
\item $f=\sum_{i=1}^n\omega_{x_i,y_i}$ for some $(x_i)_{i=1}^n,(y_i)_{i=1}^n\subset H$.
\end{parts}
\end{prb}
\begin{pf}
(2)$\impl$(3) is the only nontrivial implication.
By the definition of the strong operator topology, there exists $v\in \cH^n$ such that
\[|f(T)|\le\|T^{\oplus n}v\|.\]
...
For $(x_i)_{i=1}^\infty,(y_i)_{i=1}^\infty\subset H$ such that $\sum_i\|x_i\|^2<\infty$ and $\sum_i\|y_i\|^2<\infty$,
\[\left(\sum_i\|Tx_i\|^2+\|T^*x_i\|^2\right)^{\frac12}\qquad\left(\sum_i\|Tx_i\|^2\right)^{\frac12}\qquad\left|\sum_i\<Tx_i,y_i\>\right|\]
...
\item $U(H)$ is neither complete nor locally compact.
\end{parts}
\end{prb}
...
\begin{prb}[Symmetric extensions]
\,
\begin{parts}
...
A \emph{character} of $A$ is a non-trivial algebra homomorphism $\f:A\to\C$.
Denote by $\sigma(A)$ the set of all characters of $A$.
We will show that all characters are bounded.
Then, endow with the weak$^*$ topology on $\sigma(A)$ from the inclusion $\sigma(A)\subset A^*$.
We call this space as the \emph{spectrum} of $A$.
Let $\f\in\sigma(A)$.
\begin{parts}
\item $\|\f\|=1$.
\item If $A$ is unital, then $\sigma(A)$ is compact and Hausdorff.
\item Even if $A$ is non-unital, $\sigma(A)$ is locally compact and Hausdorff.
\end{parts}
\end{prb}
...
\[\Gamma:A\to C_0(\sigma(A)).\]
\begin{parts}
\item $\Gamma(A)$ separates points.
\item $\Gamma$ has closed range if
\item $\Gamma$ is injective if
\item $\Gamma$ is isometric if $r(a)=\|a\|$ for all $a\in A$.
\end{parts}
\end{prb}
...
For a commutative unital C$^*$-algebra $A$, consider the Gelfand transform $\Gamma:A\to C(\sigma(A))$.
\begin{parts}
...
Let $A$ be a C$^*$-algebra, and $a\in A$ a normal element.
...
A \emph{representation} of $A$ is a $*$-homomorphism $\pi:A\to B(H)$ for a Hilbert space $H$.
We say a representation $\pi:A\to B(H)$ is \emph{non-degenerate} if $\pi(A)H$ is dense in $H$, \emph{cyclic} if there is $\psi\in H$ such that $A\psi$ is dense in $H$, and \emph{irreducible} if there is no proper closed subspace $K\subset H$ such that $\pi(A)K\subset K$.
\begin{parts}
...
\[PS(A)\to\hat A\to\operatorname{Prim}(A).\]
\end{prb}
...
Let $\tilde\rho=\rho\oplus\lambda$ be a bounded linear functional on $\tilde A$, where $\rho\in A^*$ and $\lambda\in\C^*=\C$.
\begin{parts}
...
Let $H$ be a Hilbert space.
A $*$-subalgebra $M$ of $B(H)$ is called a \emph{von Neumann algebra} if it is closed weakly.
\end{prb}
...
Let $M\subset B(H)$ be a von Neumann algebra.
\[B^\infty(\sigma(a))\to M.\]
\begin{parts}
...
\begin{parts}
\item $\bar A^{\sigma s^*}\subset\bar A^w\subset A''$.
\item If $x\in A''$, for any $\e>0$ and $\xi\in H$ there is $a\in A$ such that $\|(x-a)\xi\|<\e$.\\
(If we can find such $a\in A$ for any \emph{finite subset} $F\subset H$ not only for a single $\xi$, then we can construct a net $a_\alpha$ that converges to $x$ strongly, i.e.~$\bar A^s=A''$. We will show, more strongly, that we can do this for any \emph{square-summable countable subset} $F$ in the part (c))
\item For $\bar A^{\sigma s^*}= A''$.
\end{parts}
\end{prb}
\begin{pf}
(b)
...
Then, the image $apH$ is contained in $\bar{A\xi}$, we have $pap=ap$ and $pa^*p=a^*p$ for all $a\in A$ by the self-adjointness of $\cA$.
It implies $ap=pa$, which deduces $p\in A'$ so $xp=px$.
Observe that $a(1-p)\xi=(1-p)a\xi=0$ for all $a\in A$.
Then, $\<(1-p)\xi,\eta\>$ for any $\eta\in H=\bar{AH}$, so $p\xi=\xi$.
Hence $x\xi=xp\xi=px\xi$ so that $x\xi\in\bar{A\xi}$.

(c)
We suffices to show $A''\subset\bar A^{\sigma s}$ because $A$ is self-adjoint.
Take a finite set $\{(\xi_{ij})_{j=1}^\infty\subset H\}_{i=1}^n$ of sequences such that $\sum_{j=1}^\infty\|\xi_{ij}\|^2<\infty$ for each $i$.
Then, $x\mapsto\left(\sum_{j=1}^\infty\|x\xi_{ij}\|^2\right)^{\frac12}$ defines a finite set of seminorms indexed by $i$ which makes a base element of the $\sigma$-strong topology.
Consider the diagonal map $\Delta:B(H)\to B(H^{\oplus\infty})$ and let $\bar\xi:=(\xi_{ij})_{i,j}\in(H^{\oplus\infty})^{\oplus n}=H^{\oplus\infty}$.
Then, the seminorm for $\sigma$-strong topology on $H$ factor through the seminorm defined by $\xi$ on $H^{\oplus\infty}$ as follows:
\[\begin{tikzcd}[column sep=huge]
B(H) \ar[swap]{d}{\Delta} \ar{dr}{x\mapsto\left(\sum_{i,j}\|x\xi_{ij}\|^2\right)^{\frac12}} & \\
B(H^{\oplus\infty}) \ar[swap]{r}{\bar x\mapsto\|\bar x\bar\xi\|} & \R_{\ge0}.
\end{tikzcd}\]

Suppose $x\in A''$.
Since $\Delta(x)\in\Delta(A)''$ and $\Delta(A)$ is a non-degnerate $*$-subalgebra of $B(H^{\oplus\infty})$, by the part (b), there is $\Delta(a)\in\Delta(A)$ such that
\[\|(\Delta(x)-\Delta(a))\bar\xi\|=\left(\sum_{i,j}\|(x-a)\xi_{ij}\|^2\right)^{\frac12}<\e.\]
Thus
\[\Delta(A'')\subset\Delta(A)''\subset\Delta(\bar A^{\sigma s}).\]
\end{pf}
...
constructed by adjoints, where $M(\pi_u)_*$ denotes the set of normal linear functionals on $M(\pi_u)$.
\begin{parts}
\item $\pi_u^*$ is isometric.
\item $\pi_u^*$ is surjective.
\item $\pi_u^{**}$ is an isometric isomorhpism with respect to norms, and is an homeomorphism with respect to weak$^*$-topologies.
\item $A^{**}$ enjoys a universal property in the sense that for every $*$-homomorphism $\f:A\to M$ to a von Neumann algebra $M$, there exists a unique normal extension $\tilde\f:A^{**}\to M$ of $\f$.
\end{parts}
\end{prb}
\begin{pf}
(a)
...
(b)
The injective $*$-homomorphism $\pi_u$ is isometric so that its dual $M(\pi_u)^*\to A^*$ is surjective by the Hahn-Banach extension, however, it does not guarantee that the extended linear functional is normal.
We claim that every state of $A$ has a normal extension on $M(\pi_u)$.
...
If we let $\psi$ be the canonical cyclic vector of the GNS representation $\pi_\omega:A\to B(H_\omega)$, then the state $\omega$ can be represented as a vector state $\omega_\psi$ in $B(H)$.
Since $\pi_\omega$ is a subrepresentation of $\pi_u$, the unit vector $\psi$ can be seen as an element of $H_u$, and it defines a normal state of $M(\pi_u)$.
...
Let $A$ be a closed subalgebra of a C$^*$-algebra $B$ with $1_B\in A$.
A \emph{conditional expectation} is a positive $A$-bimodule map $\f:B\to A$.
\begin{parts}
\item
\item (Tomiyama theorem) contractive idempotent linear map $\f:B\to A$ is an $A$-bimodule map.
