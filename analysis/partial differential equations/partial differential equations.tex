...
\begin{prb}
We introduce the \emph{Sobolev regularity} $s/d-1/p$ for a triple of $(s,d,p)\in\R\times\Z_{>0}\times[1,\infty]$, and the \emph{H\"older regularity} $(k+\alpha)/d$ for a triple $(k,\alpha,d)\in\Z_{\ge0}\times[0,1)\times\Z_{>0}$.
\end{prb}

\begin{prb}[Gagliardo-Nirenberg-Sobolev inequality]
for $(p,d)\in[1,\infty]\times\Z_{>0}$
for $q\in[1,\infty]$,
\begin{parts}
\item If $p<d$ and $-1/q=1/d-1/p$, then
\[\|u\|_{L^q(\R^d)}\lesssim\|u\|_{\dot W^{1,1}(\R^d)},\qquad u\in C_c^\infty(\R^d).\]
\end{parts}
\end{prb}
\begin{proof}
(a)
We first show $W^{1,1}\to L^q$ for $1<d$, where $q:=\frac d{1-d}$, using the averaging argument by Gagliardo.
Fix $u\in C_c^\infty(\R^d)$.
Define a smooth bounded functions $F_\kappa$ on $\R^d$ for $\kappa\subset\{1,\cdots,d\}$ such that
\[F_\kappa(x):=\left(\int_{\R^{|\kappa|}}|\nabla u(x)|\,dx_\kappa\right)^{\frac1{d-1}},\qquad x\in\R^d.\]
Then,
\[|u(x)|\le\int_\R|\partial_iu(x)|\,dx_i\le\int_\R|\nabla u(x)|\,dx_i=F_i(x)^{d-1},\qquad x\in\R^d,\ 0\le i\le d,\]
implies $|u|^q\le F_1\cdots F_d$.
Since $F_\kappa$ is constant along the direction $x_i$ if $i\in\kappa$, the H\"older inequalities get
\[\int_\R|u|^q\,dx_1\le F_1\int_\R F_2\cdots F_d\,dx_1\le F_1\prod_{i=2}^d\left(\int_\R F_i^{d-1}\,dx_1\right)^{\frac1{d-1}}=F_1F_{12}\cdots F_{1d},\]
and
\[\int_{\R^2}|u|^q\,dx_{12}\le F_{12}\int_\R F_1F_{13}\cdots F_{1d}\,dx_2\le F_{12}^2F_{123}\cdots F_{12d},\]
which continues inductively and leads finally
\[\int_{\R^d}|u|^q\,dx\le F_{1\cdots d}^d=\left(\int_{\R^d}|\nabla u(x)|\,dx\right)^q,\]
so
\[\|u\|_{L^q(\R^d)}\lesssim\|u\|_{\dot W^{1,1}(\R^d)}.\]

Now we prove $W^{1,p}\to L^q$ for $1<p<d$ and $q:=\frac{d-p}{pd}$.
If we choose $\gamma>0$ such that
