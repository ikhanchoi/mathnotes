...
\section{Definition of smooth structures}


\begin{prb}[Locally Euclidean spaces]
continuous atlas
\begin{parts}
...
\begin{parts}
\item Long line
\item Line with two origins
\end{parts}
\end{prb}

\begin{prb}[Smooth maps]
\end{prb}

\begin{prb}[Diffeomorphisms]
\end{prb}
...
\begin{prb}[Scalar fields and vector fields]
...
\end{prb}
\begin{prb}[Tagent space of matrix group]
Jacobi formula
\end{prb}
...
\begin{cd}
f^{-1}(q)\cap U \dar{\f|_{f^{-1}(q)\cap U}} & U \rar{f}\dar{\f} & V \dar{\psi} \\
\ker\tilde f\cap\f(U) & \f(U) \rar{\tilde f} & \psi(V)
\end{cd}
