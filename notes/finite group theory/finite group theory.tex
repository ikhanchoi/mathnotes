...
\begin{ex}[$|G|=p^2q$]$ $\\[-10pt]

Let $1+p<q$.
Then, $n_q=1$.
\begin{cond}
\item \[\f:Z_{p^2}\to\Aut(Z_q)\cong Z_{q-1}.\]
There are $\min\{2,v_p(q-1)\}$ nonabelian groups.
\item \[\f:Z_p\x Z_p\to\Aut(Z_q)\cong Z_{q-1}.\]
There are $\min\{1,v_p(q-1)\}$ nonabelian groups.
\end{cond}

Let $p>q$.
Then, $n_p=1$.
\begin{cond}
\item $G\cong Z_{p^2}\rtimes Z_q$.
Consider
\[\f:Z_q\to\Aut(Z_{p^2})\cong Z_{p(p-1)}.\]
There are $\min\{1,v_q(p-1)\}$ nonabelian groups.
\item $G\cong(Z_p\x Z_p)\rtimes Z_q$.
Consider
\[\f:Z_q\to\Aut(Z_p\x Z_p)\cong\GL_2(\F_p).\]
There are
\[\begin{cases}2&,q=2\\1&,q\mid p+1\\\frac{q+3}2&,q\mid p-1\\0&,\text{otherwise}\end{cases}\]
nonabelian groups.
