...
The above theorem deeply related to Schur's lemma in representation theory, which means it holds for not only fields but also division rings.
...
Since $F[x]$ is PID, every prime ideal is maximal (Krull dimension 1), and every irreducible element generates a nonzero prime ideal (Euclid's lemma).
...
In particular, this proposition implies that algebraically closedness can be described by the factorization in itself so that it is an internal property; it is preserved under isomorphisms.

Algebraic closures are defined as follows
...
We have $\cl F<K$ since every algebraic element $\alpha$ with $f(\alpha)=0$ for $f\in F[x]\subset K[x]$ should be contained in $K$.
...
\begin{lem}[Frobenius automorphism]
...
\end{defn}
This definition is independent on the base field $F$ whenever $f\in F[x]$.
\begin{defn}
