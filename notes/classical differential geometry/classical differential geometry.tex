...
In general, vector fields are basically assumed to be tangent.
...
% Differentiability is independent on the choice of parametrizations!!!
...
\begin{prop} % Einstein convention!!!
...
\begin{align*}
\pd_XY-\pd_YX
&=\pd_X(Y^j\a_j)-\pd_Y(X^i\a_i)\\
&=[(\pd_XY^j)\a_j+Y^j\pd_X\a_j]-[(\pd_YX^i)\a_i+X^i\pd_Y\a_i]\\
&=[(\pd_XY^j)\a_j+Y^jX^i\pd_i\a_j]-[(\pd_YX^i)\a_i+X^iY^j\pd_i\a_j]\\
&=(\pd_XY^j)\a_j-(\pd_YX^i)\a_i.\qedhere
\end{align*}
\end{pf}
...
\begin{thm}
Let $X,Y$ be linearly independent tangent vector fields on a regular surface $S$.
If $\pd_XYf=\pd_YX$, then $S$ admits a parametrization $\a$ such that $\a_x|_p=X|_p$ and $\a_y|=Y|_p$ for each $p\in S$.
\end{thm}
...
Then, $g_{ik}\,\cS_j^k=L_{ij}$ since
\[g_{ik}X^i\cS_j^kY^j=\<X,\cS(Y)\>=\<\pd_XY,\nu\>=X^iY^jL_{ij}.\qedhere\]
\end{pf}
