...
We also just often say that $\a$ is a regular curve (resp. regular surface) for a particular parametrization $\a$.
...
Coordinates and parametrizations give equivalent information except that the direction is opposite (only if parametrization satisfies the injectivity).
...
As said, the choice of coordinate(parametrization) is extremely important in differential geometry.

% Examples

When developing a theory of differential geometry, the followings are considered to be important issues:
\begin{itemize}
\item Well-definedness of a structure with respect to the dependency on parametrizations(coordinates).
\item Existence of a coordinate system that has nice properties we want.
\end{itemize}
...
\[\rT'=\|\a'\|\kappa\rN,\qquad\rB'=-\|\a'\|\tau\rB.\]
...
\subsection{Problems}
We are interested in a curve, not a particular parametrization.
...
by direct differentiation since we have known $\<\a',\rT\>$ but not $\<\a,\rN\>$, and further
...
Always $\a$ denote a reparametrized unit speed nondegenerate curve in $\R^3$.
...
\begin{prb}
A curve with more than one Bertrand mates is a cylindrical helix.
\end{prb}
\begin{sol}
\Step{1}[Formulate conditions]
...
The problem can be solved by dividing the cases, but in this solution we give only for the case that $\a$ is not planar; the other hand is easy.
...
The inner product on $T_pS$ induced from the standatd inner product of $\R^3$ can be represented as a matrix
