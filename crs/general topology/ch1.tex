...
It is often required to consider combining a possibly uncountable family of generalized metrics, called pseudometrics.
...
Every statement and concept in 1.3.1, topological equivalence of metrics, except Proposition \ref{prop:sequential convergence determines equivalent metrics} is extended to the pseudometrics.
...
\end{prb}
\begin{prb}
Show that there is a metric $d$ on $\R$ such that a sequence $x_n=\frac1n$ converges to 1 and a sequence $y_n=1-\frac1n$ converges to 0.
\end{prb}
\begin{prb}
...
\end{prb}
\begin{prb}
Let $\|\cdot\|$ and $\|\cdot\|'$ be norms on a vector space $X$.
Suppose that a sequence $\{x_n\}_n$ in $X$ converges to $x$ in $\|\cdot\|$ and converges to $x'$ in $\|\cdot\|'$.
Show that $x=x'$.
\end{prb}
\begin{prb}
...
\end{prb}
\begin{prb}
Compute the range of the function $f(x)=\|x\|_p/\|x\|_q$ defined for $x\in\R^d$, where $\|x\|_p^p:=\sum_{i=1}^d|x_i|^p$ for $x=(x_1,\cdots,x_d)$.
\end{prb}
