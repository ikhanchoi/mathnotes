...
Let $f:[0,\oo)\to[0,\oo)$ be a strictly increasing function such that $f(0)=0$.
...
The two metrics are called \emph{topologically equivalent} if the set of open balls are mutually nested;
for arbitrary $\e>0$, we can find $\e_1>0$ and $\e_2>0$ such that
\[B_1(x,\e_1)\subset B_2(x,\e)\quad\text{and}\quad B_2(x,\e_2)\subset B_1(x,\e),\]
...
\[d_1(x,y)\le C_1d_2(x,y)\quad\text{and}\quad d_2(x,y)\le C_2d_1(x,y)\]
...
\begin{thm}
Let $d$ be a metric on a set $X$ and let $f$ be a strictly increasing subadditive real function on $\R_{\ge0}$ such that $f(0)=0$ so that $f\o d$ is a metric.
If $f$ is continuous in addition, then $f\o d$ is equivalent to $d$.
\end{thm}
\begin{pf}
%%%
\end{pf}
\begin{cor}
Let $f:[0,\oo)\to[0,\oo)$ be a continuous function.
If
\[f(0)=0,\quad f'(x)>0,\quad f''(x)\le0\]
for all $x>0$, then $f\o d$ and $d$ are equivalent metrics where $d$ is a metric on a set.
\end{cor}
...
However, the checking procedure becomes rather simple.
...
\begin{thm}
Every norm on a finite dimensional vector space is equivalent.
\end{thm}
\begin{pf}
%%%
\end{pf}
