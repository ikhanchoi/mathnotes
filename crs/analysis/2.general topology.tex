...
We assume the reader is already familiar to the theory of normed spaces.
...
Moreover, every subset of normed space is also an example of metric space because metric can be inherited to every subset of  metric space.

\begin{ex}
A normed space $X$ is a metric space.
Precisely, the norm structure naturally induces a real-valued function $d$ on $X\x X$ defined by $d(x,y):=\|x-y\|$ and it satisfies the axioms of metric.
\end{ex}
\begin{pf}
It is quite easy.
...
To state the theorem, we introduce isometries.
...
If two metric spaces are isometric, we can view them as virtually same, in the sense of metrics.
...
Many freshmen misunderstand, for its name, that metric is something belonging to the study of geoemtry.
...
Now let us reformulate the definitions of limits and continuity with balls, which we actually use in the usual calculus on Euclidean spaces.
Compare the following definitions to what you remember.
...
Note that taking $\e$ or $\delta$ really means taking a ball of the very size.
