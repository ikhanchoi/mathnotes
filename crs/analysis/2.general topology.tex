...
According to the usage of topologies, similarly as mentioned, there are two large branches of general topology; both contribute to build nice frameworks for wide regions of mathematics.
...
\iffalse
For generalization, we firstly come up with the following question: exactly what aspects of norms could have made them used to define the notion of convergence?
...
($\Leftarrow$)
% exercise ? ?? cut?? 1.2? Hausdorffness? ?? ?? ??? ?
So, suppose a sequence $\{x_n\}_n$ converges to a common point $x$ with respect to both $d_1$ and $d_2$.
...
Every metric can be bounded above by an arbitrary constant, which was 1 in the proof, keeping the topological equivalence.

\begin{prop}
Let $\{d_i\}_{i\in\N}$ be a countable family of metrics on a set $X$.
Then a function $d:X^2\to\R_{\ge0}$ defined by
\[d(x,y):=\sum_{i\in\N}\,2^{-i}d_i(x,y)\]
is a metric.
