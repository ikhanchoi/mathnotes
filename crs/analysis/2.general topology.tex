...
In this subsection, we will describe a topology generated by several metrics, and, in general, by several ``pseudometrics''.
...
\begin{prop}
Let $\{d_i\}_{i=1}^d$ be a family of metrics on a set $X$.
...
We prove only for a pair of metrics, but it is easy to check by mathematical induction that any finite family of metrics can be combined to make new metrics, which are essentially equivalent.
...
% A topology generated by countable family of metrics is metrizable.
...
In fact, the metric combining technique is used for a family of metrics once in a while; it is frequently used for a family of pseudometrics, which has not been defined yet.
At the last of this section, we give a typical example that requires a metric constructed by bounding and combining pseudometrics.
