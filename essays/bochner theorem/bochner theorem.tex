...
The corresponding positive definite sequence is
\end{ex}
...
We have omitted the regularity in Chapter 2 because every finite Borel measure on a compact metric space is regular in this sense.
...
\end{pf}
\begin{ex}[Space of finite measures]
Two topologies are different if we change the space of probability measures to the space of finite measures.
A terse example is the shifting sequence of dirac measures $\delta_n$, which converges vaguely to the zero measure, but diverges weakly.
\end{ex}

Thanks to this theorem, under the assumption that the base space is locally compact and Hausdorff, we have no need to distinguish the topology of weak convergence and the weak$^*$ topology.
...
Let $M$ be a set of probability measures on a metric space $S$.
...
Let $M$ be a set of probability measures on a metric space $S$.
The set $M$ is relatively compact in the topology of weak convergence.
\end{thm}
