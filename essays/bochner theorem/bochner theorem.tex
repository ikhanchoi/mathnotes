...
We give representation-theoretic interpretations of Fourier transform and adjoint Fourier transform.
\begin{itemize}
\item The Fourier transform is itself a unitary representation.
\item Each element of $M(\hat G)$ corresponds
\end{itemize}
...
Second, each element of the measure space $M(\hat G)$ is considered as a representation of $G$.



\begin{cd}
G\rar{\Phi}\dar[hook]&\hhat G\dar[hook]\\
M(G)\rar{\cF^*}&\prod\limits_{[\pi]\in\hat G}\End H_\pi.
\end{cd}
...
Then, since $\cF^*\f(y)=\int\Phi^*(y)(\chi)\f(\chi)\,d\chi$, so $\Phi^*(x_\alpha)$ does not converges to $\Phi^*(x)$ in the weak$^*$ topology of $L^\infty(\hat G)$, which is same as the topology on $\hhat G=(L^1(\hat G))\hat\ $.

(Closedness)
Let $y\in\bar{\Phi(G)}$ such that there is a net $x_\alpha\in G$ satisfying $\Phi(x_\alpha)\to y$ in $\hhat G$.
