...
\begingroup%
  ...
  \hat{\raisebox{.35ex}{\vphantom{#1}}\smash{\hat{#1}}}%
\endgroup%
}
\makeatother
...
This rotating exponential term can be recognized as continuous group homomorphism from $G$ to circle group $\T$ in general, so we will introduce the notion of space of these homomorphisms and define Fourier transform on $G$ by using this.
...
%When we see a function $L^1(G)$ as a density function on $G$, then the Fourier transform is nothing but a infinitesimal linear combination of characters.
...
As in vector spaces, there is a natural homomorphism from a group to its double dual group.

\begin{defn}[Canonical homomorphism]
Let $G$ be a locally compact abelian group, and $\hhat G$ be its double dual group.
We call the map $\alpha:G\to\hhat G$ defined such that $\alpha(x)(\chi):=\chi(x)$ as the \emph{canonical homomorphism} of $G$.
\end{defn}
...
on $L^1(G)$, which is a non-zero algebra homomorphism, so induces a map $\hat G\to(L^1(G))\hat\ $.
...
\begin{prop}
The map $\hat G\to(L^1(G))\hat\ $ is a homeomorphism.
\end{prop}
\begin{pf}
...
We will also often use the adjoint Fourier transform $\cF^*:M(\hat G)\to\C^{\hat{\hat G}}$ defined by
...
Note that we can embed $G$ into the algebra $M(G)$ as a dirac measures.
One way of recognizing Fourier transform is a lifting of the canonical homomorphism $\alpha:G\to\hhat G$, as described in the following commutative diagram:
\begin{cd}
G\rar{\alpha}\dar[hook]&\hhat G\dar[hook]\\
M(G)\rar{\cF^*}&\C^{\hat G}
\end{cd}
...
\begin{cor}
Let $G$ be a locally compact abelian group, and $\alpha:G\to\hhat G$ be the canonical homomorphism.
Then, $\alpha^*\circ\cF^*:M(\hat G)\to B(G)$ is a well-defined algebra isomorphism.
\end{cor}
...
We rigorouslt state them as follows.

\begin{thm}[Fourier inversion]
Let $G$ be a locally compact abelian group, and $\hat G$ be its dual group.
By adjusting the constant of Haar measure on $\hat G$, we have the following statements:
\begin{parts}
\item For $f\in B(G)\cap L^1(G)$, we have $\cF f\in L^1(\hat G)$ and $\alpha^*\circ\cF^*\circ\cF f=f$.
\item For all $\f\in B(\hat G)\cap L^1(\hat G)$, we have $\cF^*\circ\alpha^*\circ\cF\f=\f$
\end{parts}
\end{thm}
% Explain why $\alpha^*$ is well defined: the continuity of functions
\begin{pf}
(a)
...
By the Bochner theorem, there is a non-negative measure $\mu\in M(\hat G)$ such that $\alpha^*\circ\cF^*\mu=f$.
...
(b)
Note that we can slightly modify the Bochner theorem to state that $\alpha^*\circ\cF:M(\hhat G)\to B(\hat G)$ is also an algebra isomorphism.
From the part (a), we have $\cF\f\in L^1(\hhat G)$ so that $\alpha^*\circ\cF\f\in B(G)\cap L^1(G)$ and
\[\alpha^*\circ\cF\circ(\cF^*\circ\alpha^*\circ\cF)\f
=(\alpha^*\circ\cF\circ\cF^*)\circ\alpha^*\circ\cF\f=\alpha^*\circ\cF^*\f,\]
hence $\cF^*\circ\alpha^*\circ\cF\f=\f$ by the injectivity of $\alpha^*\circ\cF$.
\end{pf}
...
Let $G$ be a locally compact abelian group, and $\hat G$ be its dual group.
Then, the canonical homomorphism $\alpha:G\to\hhat G$ is a topological isomorphism.
\end{thm}
\begin{pf}
(Embedding)
...
(Closedness)
We will show $\alpha(G)$ is closed in $\hhat G$.

(Density)
Suppose $\alpha(G)$ is not dense in $\hhat G$.
We claim that there is a non-zero function $f\in\cF^*(M(\hat G))$ on $\hhat G$ that vanishes on $\alpha(G)$.
If the claim is true, then for $\mu\in M(\hat G)$ such that $f=\cF^*\mu$ we have
\[\alpha^*\circ\cF^*\mu=0,\]
so $\mu=0$ by the Bochner theorem, and it leads a contradiction to $f\ne0$.

Let $z\in\hhat G\setminus\bar{\alpha(G)}$ and $U$ an open set in $\hhat G$ such that $zUU\cap\alpha(G)=\varnothing$.
Let $g$ be any non-negative non-zero continuous functions supported on $U$, and define $f:=(L_zg)*g$, where $L_zg(x)=g(z^{-1}x)$.
Then, $f(x)>0$ implies there are $x'$ and $x''$ in $\hhat G$ such that $x=x'x''$, $L_zg(x')>0$, and $g(x'')>0$.
It concludes $f$ vanishes on $\alpha(G)$ because $x=x'x''\in zUU\subset\alpha(G)^c$.

Let $\beta$ be the canonical homomorphism of $\hat G$.
By the Hausdorff Young inequality we have $\beta^*\circ\cF(L_zg)$ and $\beta^*\circ\cF g$ are in $B(\hat G)\cap L^2(\hat G)$.
Then,
\begin{align*}
\beta^*\circ\cF f(\chi)
&=\int_{\hat{\hat G}}x(\chi)f(x)\,dx\\
&=\int_{\hat{\hat G}}x(\chi)\int_{\hat{\hat G}}L_zg(y)g(y^{-1}x)\,dy\,dx\\
&=\int_{\hat{\hat G}}L_zg(y)\int_{\hat{\hat G}}x(\chi)g(y^{-1}x)\,dx\,dy\\
&=\int_{\hat{\hat G}}L_zg(y)\int_{\hat{\hat G}}y(\chi)x(\chi)g(x)\,dx\,dy\\
&=\int_{\hat{\hat G}}y(\chi)L_zg(y)\,dy\int_{\hat{\hat G}}x(\chi)g(x)\,dx
=\beta^*\circ\cF(L_zg)(\chi)\cdot\beta^*\circ\cF g(\chi)
\end{align*}
for all $\chi\in\hat G$, so $\beta^*\circ\cF f$ belongs to $B(\hat G)\cap L^1(\hat G)$ by the H\"older inequality, hence the inversion theorem proves $f=\cF^*\circ\beta^*\circ\cF f$ is contained in $\cF^*(L^1(\hat G))\subset\cF^*(M(\hat G))$.
\end{pf}
