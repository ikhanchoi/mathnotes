...
\begin{prb}
Consider a convex polygon $P$ in $\H^2$.
The vertices are denoted by $v_0,v_1,\cdots,v_n=v_0$, indexed along the boundary counterclockwise.
Denote by $s_i$ the side connecting $v_i$ and $v_{i+1}$.
\end{prb}

\begin{prb}[Dirichlet domain]

A point $v\in\partial D$ is called a \emph{vertex} if
\end{prb}
...
Let $\Gamma$ be a discrete group of $\Isom^+(\H^2)$ and suppose $D$ is a Dirichlet domain of $\Gamma$.
Let $V$ be 
\begin{parts}
\item For each side $s_i$ of $D$, there is $g_i\in\Gamma$ such that $g_i(\bar D)\cap\bar D=s_i$. The isometry $g_i$ is called the \emph{side pairing isometry} of the side $s_i$.
\item The side pairing isometry $g_i$ is unique if $v_i$ is 
\item Define  $v_{j+1}$, where $g_i^{-1}(s_i)=s_j$,
\end{parts}
\end{prb}
