...
\begin{prb}[Boundary and edges of Dirichlet domain]
...
\begin{prb}[Side pairing condition]
...
\begin{prb}[Genus two surface]

\end{prb}

\begin{prb}[Modular group]
...
\begin{prb}[Definition of polygons]
\begin{parts}
\item
\end{parts}
\end{prb}
...
Consider
\[p:(\Gamma\times\bar D)/\sim\to\H^2.\]
\begin{parts}
\item $p$ is well-defined.
\item $\im p$ is open.
\item $p$ is a local homeomorphism if and only if elliptic cycle condition is satisfied.
\end{parts}
\end{prb}

\begin{prb}[Parabolic cycle condition]
$\pi:\bar D\to\bar D/\sim$.
Here, we assume the word parabolic cycle condition includes finite lenth cycle condition.
\begin{parts}
\item If each cycle has finite length, then we can induce a metric on $\bar D/\sim$ given by
\[\rho(x,y):=\inf\sum.\]
\item $\pi z_n\to\pi z$ in $\bar D/\sim$ if and only if there are $h_n\in\Gamma$ such that $h_nz_n\to z$ in $\H^2$.
\item If each $\bar D/\sim$ is complete if and only if parabolic cycle condition is satisfied.
\end{parts}
\end{prb}
\begin{pf}
(b)
\[\inf_{h\in\Gamma}d(h^{-1}x,y)\le\rho(x,y).\]
\end{pf}

\begin{prb}[Proof of the Poincar\'e polygon theorem]
Let $D$ be a polygon with a side pairing identification such that elliptic and parabolic cycle conditions are satisfied.
Let $\Gamma$ be a subgroup of $\Isom^+(\H^2)$ generated by side pairing isometries of $D$.
\begin{cd}
\Gamma\times\bar D \dar\rar{\pi} & (\Gamma\times\bar D)/\sim \dar{s}\rar{p} & \H^2 \dar\\
\bar D \rar{\pi} & \bar D/\sim \rar & \H^2/\Gamma
\end{cd}
\begin{parts}
\item If $D$ satisfies the parabolic condition, then $p$ is surjective.
\item If $D$ satisfies the elliptic condition, then $p$ is injective.\end{parts}
\end{prb}
\begin{pf}
(a)
We claim that $\im p$ is closed to verify $\im p=\H^2$ with the connectedness of $\H^2$.
Let $w\in\partial(\im p)$ so that we have sequences $g_n\in\Gamma$ and $z_n\in\bar D$ such that $g_nz_n\to w$ in $\H^2$.
Since $p\pi(g_n,z_n)=g_nz_n$ is Cauchy, $s\pi(g_n,z_n)=\pi(z_n)$ is Cauchy, so we have a limit $\pi(z_n)\to\pi(z)$ in $D/\sim$ for some $z\in\bar D$.
...
Since $\im p$ is open and $\bar D\subset\im p$, there is $\e>0$ such that $B(z,\e)\subset\im p$.
There is $g\in\H^2$ such that $d(gz,w)<\e$, which implies $g^{-1}w\in B(z,\e)$.
Because $\Gamma$ acts on $\im p$, we can conclude $w\in\im p$.


(b)
We claim $p$ has the path lifting property, which is unique because it is a local homeomorphism.
Let $w:[0,1]\to\im p$, and let $\tilde w:[0,\tau)\to(\Gamma\times\bar D)/\sim$ be its maximal extension.

Let $\tilde w(t)=\pi(g(t),z(t))$ and $w(\tau)=gz$.
Define $\tilde w(\tau):=\pi(g,z)$.
Then,
\[p\tilde w(\tau)=p\pi(g,z)=gz=w(\tau).\]
Let $U$ be an open neighborhood of $\pi(g,z)$ such that $p|_U$ is a homeomorphism and $p(U)$ is open in $\H^2$.
Then, as $t\to\tau$,
...
Then, $p$ is a covering map onto its image.
Since elliptic points are at most countably many, there is $z\in D$ that has trivial stabilizer in $\Gamma$.
Then we can show the covering has single sheet.
...
\emph{Model geometry} is a $G$-space $X$ that is simply connected, transitive, and has compact stabilizers.
We only conisder \emph{maximal} model geometries.
Is the action analytic?

MAIN GOAL: We want to establish surjectivity of a map from torsion-free discrete subgroups of $G$ to complete $(G,X)$-manifolds.
...
\end{defn}
\begin{defn}[Ehresman connection]
