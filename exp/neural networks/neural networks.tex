...
\section{Motivation: statistical mechanics}
...
for $s\le t$.
Since $\cS$ is finite, we may rewrite it by
...
At each particle $n\in V$, an $\cS$-valued random variable denoted by $X_n=(x_n,p_n)$ is attached.
Fix two constants $m>0$ and $\beta:=\frac1{k_BT}>0$, and define the energy function $E:\cS\to\R$ with
\[E(x,p):=\frac{|p|^2}{2m}.\]

The assumption for Boltzmann factors is as follows:
given the reciprocal thermodynamic temperature $\beta$
\[\Pr(X_n=i)\propto e^{-\beta E_i},\]
where $E_i:=E(x_i,p_i)$.
Thus, we can write
\[\Pr(X_n=i)=\frac{e^{-\beta E_i}}{\sum_{j=1}^Ne^{-\beta E_j}}=:\frac1Z\,e^{-\frac\beta{2m}|p_i|^2}.\]
The function $Z$ is called \emph{partition function}.
Note that it depends on the data of $E$ and $\beta$.
\end{ex}
