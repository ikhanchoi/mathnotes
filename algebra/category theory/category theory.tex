...
Consequently, a set may refer to a large set, and $\Set$ now denotes the category of small sets.
...
\begin{parts}
\item There is a category $\Set$ whose objects are small sets and morphisms are functions.
...
Let $\cC$ be a locally small category, and let $F:\cC^\op\to\Set$ be a contravariant functor.
A \emph{contravariant hom-functor} or a \emph{representable functor} is a contravariant functor $\Hom(-,B):\cC^\op\to\Set$, where $B\in\cC$.
We want to show that there is a canonical bijection $\Nat(\Hom(-,B),F)\to F(B)$ for each $B\in\cC$.

\begin{parts}
\item 
\item $\cC\to\Fun(\cC^\op,\Set):A\mapsto\Hom(-,A)$ is fully faithful. (Put $F=\Hom(-,B)$)
\end{parts}
\end{prb}
\begin{proof}

\[\Nat(\Hom_\cC(-,B),F)\to F(B):\eta\mapsto\eta_B(1_B).\]

\[F(B)\to\Nat(\Hom(-,B),F):y\mapsto\eta\]
such that $\eta_A:\Hom(A,B)\to F(A):f\mapsto F(f)(y)$.
This $\eta$ is indeed a natural transformation because $F(h)\eta_{A'}=\eta_Ah^*:\Hom(A',B)\to F(A)$ for all morphisms $f:A\to A'$ in $\cC$

Then, they are mutually inverses.
...
Let $F:\cC\to\Set$ be a functor and fix an object $A\in\Obj(\cC)$.
...
Fix a strongly inaccessible cardinal $\kappa$.

The category of all $\kappa$-small sets is denoted by $\Set_{<\kappa}$.
The first and second smallest regular cardinals are $\aleph_0$ and $\aleph_1$ respectively.
A cardinal $\kappa$ is called \emph{regular} if $\Set_{<\kappa}$ admits coproducts.
A cardinal $\kappa$ is called \emph{strongly accessible} if $\Set_{<\kappa}$ admits coproducts and power sets.


Let $\cC$ be a category, and $\kappa$ be a cardinal.
An object of $\cC$ is called \emph{$\kappa$-compact} if the corepresentable functor is $\kappa$-filtered cocontinuous, and \emph{$\kappa$-small} if the same is $\kappa$-linearly filtered cocontinuous.
An object is called \emph{small} if it is $\kappa$-compact for a small regular cardinal $\kappa$.
...
weakly saturated means that it is closed under pushouts, transfinite compositions, retracts.


A category is called \emph{locally presentable} if it is a locally small cocomplete category such that there is a set of small objects generating the category by colimits.
Let $\cC$ be a locally presentable category with a set $\cI$ of morphisms.
Then, there is a functor $T:\cC^{[1]}\to\cC^{[2]}$ such that $d_2T$ belongs to the weakly saturated class of morphisms generated by $\cI$, $d_0T$ has the right lifting property with respect to the morphisms in $\cI$, and $T$ is $\kappa$-filtered cocontinuous for a small regular cardinal $\kappa$ such that each domain and codomain of morphisms in $\cI$ is $\kappa$-compact.



A model category is called \emph{combinatorial} if it is locally presentable and cofibrantly generated.
