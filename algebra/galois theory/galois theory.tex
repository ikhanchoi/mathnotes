...
In general, we want to obtain the following information of a group:
...
Suppose a group $H$ acts on a group $N$, that is, we have a group homomorphism $\f:H\to\Aut(N)$ as its representation, where $\Aut(N)$ denotes the group of group automorphisms of $N$.
The \emph{semidirect product}, written as $N\rtimes_\f H$ or just simply $N\rtimes H$ if there is no confusion, is a group with the underlying set $N\times H$ defined such that the group structure is given by
\[(n,h)(n',h')=(n\f(h)n',hh'),\qquad(n,h)\in N\times H.\]
\begin{parts}
\item The semidirect product $N\rtimes H$ is indeed a group.
\item If $N$ is a normal subgroup and $H$ is a subgroup of a group $G$ such that $N\cap H=1$ and $NH=G$, then the group $G$ is isomorphic to $N\rtimes_\f H$, where $\f:H\to\Aut(N)$ is given by the conjugation $\f(h):n\mapsto hnh^{-1}$ for $n\in N$ and $h\in H$.
...
Define a left action of the product group $\Aut(H)\times\Aut(N)$ on the set $\Hom(H,\Aut(N))$ of group homomorphisms such that for all $\f\in\Hom(H,\Aut(N))$ and $h\in H$
\[(\eta\f)(h):=\f(\eta^{-1}(h)),\qquad(\nu\f)(h):=\nu\f(h)\nu^{-1},\qquad \eta\in\Aut(H),\ \nu\in\Aut(N).\]
Consider the following maps from the orbit space, constructed by the definition of semidirect products
\[\begin{array}{ccc}
\Hom(H,\Aut(N))/(\Aut(H)\times\Aut(N))
&\to&
\{\text{ semidirect products $N\rtimes H$ }\}/\text{ isomorphism }\\
...
\Hom(H,\Aut(N))/(\Aut(H)\times\Aut(N))
&\to&
\{\text{ subgroups of $\Aut(N)$ }\}/\text{ conjugacy }\\
{[\f]}&\mapsto&{[\im\f]}
\end{array}\]
\begin{parts}
...
\begin{parts}
\item Isomorphic types of groups $G$ such that a sequence $0\to N\to G \to H\to 0$ is exact and right split,
...
\begin{parts}
\item If a prime number $p$ divides the order of a finite group $G$, then $G$ admits a subgroup of order $p$. This is called the Cauchy theorem.
\item For a finite group $G$ and a prime number $p$, there is a Sylow $p$-subgroup of $G$.
\end{parts}
\end{prb}
\begin{pf}
(a)
We use the induction on the order of $G$.
For $|G|=1$ or $p\nmid|G|$, then the Sylow $p$-subgroup is the identity group, so it is clear.

Now for $|G|>1$ with $p\mid|G|$, suppose there is a Sylow $p$-subgroup for all finite groups order less than $|G|$.

If $p\mid|Z(G)|$, then by the Cauchy theorem for abelian groups, the center $Z(G)$ has a subgroup $P_p$ of order $p$, which is normal in $G$, so the inverse image of a Sylow $p$-subgroup of $G/P_p$ under the projection $G\to G/P_p$ is a Sylow $p$-subgroup of $G$.
If $p\nmid|Z(G)|$, then since we may assume $p\mid|G|$ and the class equation for the action of $G$ on $G$ by conjugation is written as
\[|G|=|Z(G)|+\sum_{i=1}^r[G:C_G(g_i)],\]
where $r$ is the number of non-trivial orbits and $g_i$ are representatives of each orbit(?), we have $p\nmid|G:C_G(g)|$ for some $g\in G$, and a Sylow $p$-subgroup of $C_G(g)$ is a Sylow $p$-subgroup of $G$.
Therefore, $G$ admits a Sylow $p$-subgroup, and this completes the proof.
...
Let $G$ be a finite group of order $n=p^am$ for a prime $p\nmid m$.
Let $n_p:=|\mathrm{Syl}_p(G)|$ be the number of Sylow $p$-subgroups of $G$.
\begin{parts}
...
If we let $P$ and $Q$ be Sylow $p$ and $q$-subgroup of $G$, then $P\cong C_{p^2}\text{ or }C_p^2$, and $Q\cong C_q$.
...
\end{prb}
\begin{pf}
(a) $G\cong C_q\rtimes C_{p^2}$.
\[\f:C_{p^2}\to C_{q-1}.\]
There are $1+\min\{v_p(q-1),2\}$ (conjugacy classes of) subgroups of order dividing $p^2$ in $C_{q-1}$.

(b) $G\cong C_q\rtimes C_p^2$.
\[\f:C_p^2\to C_{q-1}.\]
Since $\Aut(C_p^2)\cong\GL(2,\F_p)$ transitively acts on the set of surjective homomorphisms $C_p^2\to C_p$ by twist, there are $1+\min\{v_p(q-1),1\}$ cases.

(c) $G\cong C_{p^2}\rtimes C_q$.
\[\f:C_q\to C_{p(p-1)}.\]
There are $1+\min\{v_q(p-1),1\}$ (conjugacy classes of) subgroups of order dividing $q$ in $C_{p(p-1)}$.

(d) $G\cong C_p^2\rtimes C_q$.
\[\f:C_q\to\GL_2(\F_p).\]
To compute the number of non-trivial semidirect products, we will find all the conjugacy classes of subgroups of $\GL_2(\F_p)$ with order $q$.
Note that $|\GL_2(\F_p)|=(p^2-1)(p^2-p)=(p-1)^2p(p+1)$ so that we must have $q\mid p-1$ or $q\mid p+1$.
...
Let $A\in\GL_2(\F_p)$ be of order $q$, and $\lambda^{\pm1}\in\bar\F_p$ be eigenvalues of $A$.
...
non-abelian groups of the form $C_p^2\rtimes C_q$.
...
Four prime factors less than 100
\[\begin{tabular}{c|c|cccc|c|c}
\hline
$|G|$&16&24&40&54&56&36&60\\
\hline
$\#$ of groups&14&15&14&15&13&14&13\\
\hline
\end{tabular}\]

Five or six prime factors less than 100
...
\begin{prb}[Todd-Coxeter algorithm]
\end{prb}
\begin{prb}[Knuth-Bendixon algorithm]
\end{prb}
\begin{prb}[Computing order from its group presentation]
\end{prb}
...
We say $\alpha$ is \emph{algebraic} over $F$ if the surjective ring homomorphism $F[x]\to F[\alpha]:f\mapsto f(\alpha)$ called the \emph{evaluation} at $\alpha$ is not injective, and is \emph{transcendental} over $F$ if it is injective.
If $\alpha$ is algebraic over $F$, then the kernel of the evaluation $F[x]\to F[\alpha]$ is a prime ideal since its image $F[\alpha]$ is an integral domain, and is a maximal ideal since the domain $F[x]$ is a principal ideal domain.
Because a maximal ideal of $F[x]$ has a unique monic irreducible generator, we can define the \emph{minimal polynomial} $\mu_{\alpha,F}\in F[x]$ of $\alpha$ over $F$ as the unique monic irreducible polynomial such that the evaluation at $\alpha$ induces the isomorhpism $F[x]/(\mu_{\alpha,F})\to F[\alpha]$.
...
\begin{parts}
\item $\alpha$ is algebraic over $F$ if and only if the simple extension $F(\alpha)/F$ is finite. Moreover, we have the degree formula $[F(\alpha):F]=\deg\mu_{\alpha,F}$.
\item $\alpha,\beta\in E$ are conjugate over $F$ if and only if there is a field isomorphism $\f:F(\alpha)\to F(\beta)$ such that $\f(\alpha)=\beta$ and $\f$ fixes $F$.
\end{parts}
\end{prb}
\begin{pf}
(a)
Let $\alpha$ be algebraic over $F$.
Since the quotient $F[\alpha]$ is a field because $(\mu_{\alpha,F})$ is maximal in $F[x]$, we have $F[\alpha]=F(\alpha)$ by definition of $F(\alpha)$.
If we fix any non-zero $f\in(\mu_{\alpha,F})$, then for any element $g(\alpha)\in F(\alpha)=F[\alpha]$ with $g\in F[x]$ there exist $q,r\in F[x]$ such that $g=qf+r$ and $\deg r<\deg f$ by the Euclidean algorithm in $F[x]$, so $g(\alpha)=q(\alpha)f(\alpha)+r(\alpha)=r(\alpha)$ is a linear combination of $\{1,\alpha,\cdots,\alpha^{\deg f-1}\}$ over $F$, which gives $[F(\alpha):F]\le\deg f<\infty$.
...
The coefficients on the linear dependency relation construct a non-zero polynomial vanishing at $\alpha$.
...
\[\f:F(\alpha)\to F[x]/(\mu)\to F(\beta):\alpha\mapsto x+(\mu)\mapsto\beta.\]
It is easy to check that $\f(\alpha)=\beta$ and $\f$ fixes $F$.
The uniqueness is clear.

Conversely, suppose $\f:F(\alpha)\to F(\beta):\alpha\mapsto\beta$ is a field isomomorphism fixing $F$.
Then, $\f$ commutes with a polynomial function with coefficients in $F$.
From
\[\mu_{\alpha,F}(\beta)=\mu_{\alpha,F}(\f(\alpha))=\f(\mu_{\alpha,F}(\alpha))=\f(0)=0,\]
...
\begin{prb}[Algebraically closed fields]
...
\begin{prb}[Algebraic closures]
An \emph{algebraic closure} of a field $F$ is an algebraically closed field $\bar F$ together with an algebraic field extension $\bar F/F$.
\begin{parts}
\item
If $E/F$ is a field extension and $E$ is algebraically closed, then the set of all algebraic elements in $E$ over $F$ is an algebraic closure of $F$.
\item 
Let $\f:F_1\to F_2$ be a field isomorphism.
If $E_1/F_1$ is an algebraic extension and $\bar F_2/F_2$ is an algebraic closure, then there is a field embedding $\tilde\f:E_1\to\bar F_2$ extending $\f$.
This is called the \emph{isomorphism extension theorem}.
\item
An algebraic closure is unique up to isomorphism.
\item
...
\begin{pf}
(a)
Let $\bar F$ be the set of algebraic elements in $E$ over $F$.
The extension $\bar F/F$ is clearly algebraic.

First, we prove that $\bar F$ is a field.
...
Now we show that $\bar F$ is algebraically closed.
...
(b)
Let $S$ be the set of all pairs $(K_1,\psi)$ of a subfield $K_1\le E_1$ and a field embedding $\psi:K_1\to\bar F_2$ which extends $\f$.
The set $S$ is nonempty since $\f\in S$.
It also satisfies the chain condition since the increasing union defines the upper bound of chain.
Use the Zorn lemma on $S$ to obtain a maximal element $\tilde\f:K_1\to\bar F_2$.
If there is $\alpha\in E_1\setminus K_1$, then letting $\beta\in\bar F_2$ be a root of the pushforward polynomial $\f_*(\mu_{\alpha,F_1})\in F_2[x]$, we can construct a field embedding $K_1(\alpha)\to\bar F_2:\alpha\mapsto\beta$.
It leads a contradiction to the maximality of $\tilde\f$, so $K_1=E_1$.

(c)
Suppose there are two algebraic closures $\bar F_1, \bar F_2$ of a field $F$.
By the isomorphism extension theorem, we have a field embedding $\f:\bar F_1\to\bar F_2$ which extends the identitiy map on $F$.
Since the image $\f(\bar F_1)$ is also algebraically closed and the field extension $\bar F_2/\f(\bar F_1)$ is algebraic, we must have $\f(\bar F_1)=\bar F_2$ by the definition of algebraically closedness.
Thus, $\f$ is surjective so that it is an isomorphism.

(d)
Let $F$ be a field.
If we let $S$ be the set of all non-constant monic irreducibles in $F[x]$, then since $f(x_f)$ does not generate a constant in the polynomial ring $F[x_f]_{f\in S}$, there is a maximal ideal $\fm$ of $F[x_f]_{f\in S}$ containing $f(x_f)$ for all $f\in S$.
Then, every non-constant polynomial, with a monic irreducible factor $f$, has a root $x_f+\fm$ in the field $L_1:=F[x_f]_{f\in S}/\fm$.

As we did in the above, construct an increasing tower $L_n$ of fields inductively such that every nonconstant polynomial over $L_n$ has a root in $L_{n+1}$, and define $L:=\colim_nL_n$ as the inductive limit, which is in other word just the directed union of $L_n$ through $n\in\N$.
Then, $L$ is easily checked to be algebraically closed, so if we define $\bar F$ to be the set of all algebraic elements of $L$ over $F$, then $\bar F/F$ is an algebraic closure of $F$.

As a remark, in fact, this $L_1$ is already algebraically closed.
Since it is difficult to prove directly, so we constructed an algebraically closed field $L$ in another way.
\end{pf}
...
Let $E$ be a finite field of positive characteristic $p$ for a prime $p$.
Then, $E$ has a subfield $F$ of size $p$ additively generated by $1\in E$.
We may assume $F=\F_p$.
Since $E/F$ is trivially finite, $E$ is isomorphic to a subfield of $\bar\F_p$ by the isomorphism extension theorem.
...
\item For each $n\in\Z_{>0}$, there is a unique field of size $p^n$ in $\bar\F_p$.
\end{parts}
\end{prb}


\begin{prb}[Cyclic groups in finite fields]
The \emph{Euler totient} is a function

Let $q=p^n$.
\begin{parts}
\item The number of elements of order $d$ in a cyclic group is either zero or $\phi(d)$.
\item The group of units $\F_q^\times$ is cyclic.
\item The group of automorphisms of $\F_q$ fixing $\F_p$ is cyclic.
\end{parts}
\end{prb}
\begin{pf}
(b)
We partition the elements of $\F_q^\times$ by their orders.
Since each element of $\ord^{-1}(d):=\{\,\alpha\in\F_q^\times:\ord(\alpha)=d\,\}$, if it exists, defines a generator of the group $\mu_d(\F_q):=\{x\in\F_q:x^d=1\}$ of $d$-th roots of unity in $\F_q^\times$ whose order is at most $d$, so we have $|\ord^{-1}(d)|\in\{0,\phi(d)\}$ by the part (a).
Then, since $\ord^{-1}(d)$ is empty unless $d\mid q-1$, we have
\[|\F_q^\times|=\sum_{d\mid q-1}|\ord^{-1}(d)|\le\sum_{d\mid q-1}\phi(d)=q-1=|\F_q^\times|,\]
and it implies that $\ord^{-1}(q-1)$ is non-empty, $\F_q^\times$ is hence cyclic.
\end{pf}
...
\begin{prb}[Separable polynomials]
Let $F$ be a field.
A polynomial $f\in F[x]$ is called \emph{separable} over $F$ if it is square-free in $\bar F[x]$.
...
\begin{prb}[Straightedge and compass construction]
Regular $n$-gon
