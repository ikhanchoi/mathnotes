...
Note that since decomposition of spectrum is orginated for application to quantum mechanics, this traditional definition is usually for closed operators. Even though the following definitions can be applied for non-closable operators, but it does not make nice senses. So, every operator in this subsection is assumed to be closed.

Let $X=Y$ in order to see $L(X,Y)$ as a ring.
Let $L(X)\subset D(X)\subset B(X)$ be spaces of linear operators, densely defined operators, bounded operators respectively.
...
\[adj:D_{closable}(X)\equiv D_{cl}(X^*).\]
For $f:X\to Y$ the predicate $f:A\equiv B$ means
\[f(A)=B and A=f^{-1}(B).\]
